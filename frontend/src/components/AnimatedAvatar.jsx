...
const STYLES = {
  ...
  'sexy-goddess':    { skin: '#FFDAB9', hair: '#1a0a0a', hairHi: '#3a2020', eye: '#8B0000', outfit: { formal: ['#8B0000', '#600000'], casual: ['#FF1493', '#C71585'], tech: ['#9400D3', '#6A0DAD'], cute: ['#FF69B4', '#FF1493'] } },
  'sweet-girl':      { skin: '#FFF0E6', hair: '#8B4513', hairHi: '#D2691E', eye: '#FF69B4', outfit: { formal: ['#FFB6C1', '#FF69B4'], casual: ['#FFC0CB', '#FF91A4'], tech: ['#DDA0DD', '#BA55D3'], cute: ['#FFE4E1', '#FFC0CB'] } },
  'cool-queen':      { skin: '#F5F0EB', hair: '#0a0a0a', hairHi: '#2a2a2a', eye: '#4169E1', outfit: { formal: ['#1a1a2e', '#0d0d1a'], casual: ['#2d1b69', '#1a0f3c'], tech: ['#191970', '#0f0f40'], cute: ['#4B0082', '#2E0854'] } },
  'charming-mature': { skin: '#FFE4D6', hair: '#4A0E17', hairHi: '#6B1D23', eye: '#8B4513', outfit: { formal: ['#722F37', '#4A1B20'], casual: ['#C41E3A', '#8B1525'], tech: ['#800020', '#500015'], cute: ['#E32636', '#B22222'] } },
...
const AnimatedAvatar = forwardRef(function AnimatedAvatar({
  ...
  const drawEyes = (ctx, cx, cy, s, state, st) => {
    ...
    const eyeW = 14 * s; const irisW = 9 * s; const pupilW = 5 * s
  ...
  const drawMouth = (ctx, cx, cy, s, state) => {
    const e = state.exprCurrent
    const open = state.mouthOpen; const mouthY = cy * 0.65; const p = state.phonemeCurrent
    if (open < 0.015) {
      ...
      const mh = Math.max(2.5, open * 38) * s * p.mh; const mw = Math.max(8, 14 * s * p.mw)
      ctx.fillStyle = '#8B3333'; ctx.beginPath(); ctx.ellipse(cx, mouthY + mh * 0.3, mw, mh, 0, 0, Math.PI * 2); ctx.fill()
      if (open > 0.25) { ctx.fillStyle = '#E57373'; ctx.beginPath(); ctx.ellipse(cx, mouthY + mh * 0.5, mw * 0.55, mh * 0.45, 0, 0, Math.PI); ctx.fill() }
    }
  }
  ...
  const drawHair = (ctx, cx, cy, s, state, st, t) => {
    ...
    for (let i = 0; i < 5; i++) { const bx = cx - 35 * s + i * 18 * s; ctx.lineTo(bx - 8 * s, cy * 0.12); ctx.lineTo(bx + 8 * s, cy * 0.0) }
  ...
  const drawArm = (ctx, cx, cy, s, state, side, t) => {
    ...
    let armAngle = side * 0.3; let forearmAngle = side * 0.5
    if (gType === 0) { armAngle = side * 0.2 + Math.sin(gPhase * 0.7) * 0.1; forearmAngle = side * 0.4 + Math.sin(gPhase * 0.8) * 0.08 }
    else if (gType === 1) { armAngle = side * (-0.4) + Math.sin(gPhase * 1.5) * 0.5; forearmAngle = side * (-0.6) + Math.cos(gPhase * 1.3) * 0.4 }
    else if (gType === 2) { armAngle = side * (-0.7) + Math.sin(gPhase * 0.5) * 0.2; forearmAngle = side * (-1.0) + Math.cos(gPhase * 0.6) * 0.15 }
    ctx.strokeStyle = topColor; ctx.lineWidth = 14 * s; ctx.lineCap = 'round'
    ctx.beginPath(); ctx.moveTo(0, 0); const armX = Math.sin(armAngle) * 35 * s; const armY = Math.cos(armAngle) * 35 * s
    ctx.lineTo(armX, armY); ctx.stroke()
    ctx.translate(armX, armY)
    const faX = Math.sin(forearmAngle) * 30 * s; const faY = Math.cos(forearmAngle) * 30 * s
    ctx.lineWidth = 10 * s; ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(faX, faY); ctx.stroke()
    ctx.fillStyle = style.skin; ctx.beginPath(); ctx.arc(faX, faY, 7 * s, 0, Math.PI * 2); ctx.fill()
    ctx.restore()
  }

  // ══ 卡通模式主绘制 ══
  const drawCartoonCharacter = useCallback((ctx, w, h, state, st, t) => {
    const s = Math.min(w, h) / 480; const cx = w / 2; const cy = h / 2
    ctx.clearRect(0, 0, w, h)
    const tilt = state.headTilt * 0.04
    ctx.save(); ctx.translate(cx, cy * 0.7); ctx.rotate(tilt); ctx.translate(-cx, -cy * 0.7)
    const nod = state.nodAmount * 0.08; ctx.translate(0, nod * s * 5)
    drawBody(ctx, cx, cy, s, state, w, h, t)
    drawArm(ctx, cx, cy, s, state, -1, t); drawArm(ctx, cx, cy, s, state, 1, t)
    ctx.fillStyle = st.hairHi; ctx.beginPath(); ctx.ellipse(cx, cy * 0.55, 62 * s, 70 * s, 0, 0, Math.PI * 2); ctx.fill()
  ...
  }, [width, height, avatarId, talking, expression, outfit, scene, glasses, hat, usePortrait, getAudioData, detectPhoneme, drawCartoonCharacter, drawPortraitOverlay])
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl" style={{ width, height }}>
      ...
      <canvas ref={canvasRef} className={`absolute inset-0 ${className}`} style={{ width, height }} />
