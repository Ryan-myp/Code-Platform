...
import { Bold, Italic, Underline as UnderlineIcon, Heading2, Heading3, List, ListOrdered, Code, Type } from 'lucide-react'

export default function RichTextEditor({ value = '', onChange, placeholder = '请输入内容...', minHeight = 120 }) {
  ...
  const isActive = (cmd) => {
    try { return cmd() } catch { return false }
  }
  ...
  return (
    <div className="border border-gray-200 rounded-lg overflow-hidden focus-within:border-purple-500 focus-within:ring-2 focus-within:ring-purple-500/10 transition-all">
      <div className="flex flex-wrap gap-0 p-2 bg-gray-50 border-b border-gray-200">
        <ToolbarButton onClick={() => toggleHeading(2)} title="标题 H2" active={isActive(() => editor.isActive('heading', { level: 2 }))}>
          <Heading2 className="w-4 h-4" />
        </ToolbarButton>
        <ToolbarButton onClick={() => toggleHeading(3)} title="标题 H3" active={isActive(() => editor.isActive('heading', { level: 3 }))}>
        ...
        <ToolbarButton onClick={toggleUnderline} title="下划线 (Ctrl+U)" active={isActive(() => editor.isActive('underline'))}>
          <UnderlineIcon className="w-4 h-4" />
        </ToolbarButton>
        <span className="w-px h-6 bg-gray-300 mx-1 self-center" />
        <ToolbarButton onClick={toggleCodeBlock} title="代码块" active={isActive(() => editor.isActive('codeBlock'))}>
        ...
        <span className="w-px h-6 bg-gray-300 mx-1 self-center" />
        <ToolbarButton onClick={toggleBulletList} title="无序列表" active={isActive(() => editor.isActive('bulletList'))}>
          <List className="w-4 h-4" />
        </ToolbarButton>
        <ToolbarButton onClick={toggleOrderedList} title="有序列表" active={isActive(() => editor.isActive('orderedList'))}>
