...
import { TestTube2, Send, Loader2, CheckCircle2, AlertCircle, ListChecks, Bug, Shield, Copy, Sparkles, Bot, User, Edit2, Save, X } from 'lucide-react'
...
export default function TestPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await axios.post('/api/prd/test-cases', { prd_text: stripImages(prdText), tech_design: stripImages(techDesign) })
      const result = res.data.result || res.data.cases || '生成失败'
      setMessages(prev => [...prev,
        { role: 'user', content: prdText + (techDesign ? '\n\n技术方案: ' + techDesign : ''), timestamp: new Date().toISOString() },
  ...
  const handleChatSend = async () => {
    ...
    try {
      ...
      const res = await axios.post('/api/prd/test-cases', { prd_text: fullContext, tech_design: techDesign })
      setMessages(prev => [...prev, { role: 'assistant', content: res.data.result || res.data.cases || '处理失败', timestamp: new Date().toISOString() }])
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center space-y-4 py-6">
        <h1 className="text-3xl font-bold text-gray-900 flex items-center justify-center gap-2"><TestTube2 className="w-8 h-8 text-green-600" /> 测试用例</h1>
        <p className="text-lg text-gray-600">基于 PRD 和技术方案自动生成测试用例，支持对话式迭代优化</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-[1600px] mx-auto" style={{ height: 'calc(100vh - 280px)' }}>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
          <div className="px-5 py-3 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
            <h2 className="text-base font-semibold text-gray-900 flex items-center gap-2"><ListChecks className="w-4 h-4 text-green-600" />输入</h2>
            <span className="text-xs text-gray-400">左侧：输入需求，点击生成</span>
          </div>
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            <RichTextEditor value={prdText} onChange={setPrdText} placeholder={"请输入 PRD 内容..."} minHeight={150} />
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">技术方案（可选）</label>
              <RichTextEditor value={techDesign} onChange={setTechDesign} placeholder="粘贴技术方案内容，增强测试覆盖度..." minHeight={100} />
            </div>
            <button onClick={handleGenerate} disabled={loading || !prdText.trim()}
              className="w-full bg-gradient-to-r from-green-600 to-emerald-600 text-white py-2.5 px-4 rounded-lg hover:from-green-700 hover:to-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium transition-all flex items-center justify-center gap-2">
              {loading ? <><Loader2 className="w-4 h-4 animate-spin" />生成中...</> : <><Sparkles className="w-4 h-4" />生成测试用例</>}
            </button>
            <div className="p-3 bg-green-50 rounded-lg border border-green-100">
              <p className="text-xs font-medium text-green-700 mb-1">💡 使用提示</p>
              <ul className="text-xs text-green-600 space-y-0.5">
                <li>• 在左侧输入 PRD（可加技术方案），点击"生成测试用例"</li>
                <li>• 用例出现在右侧聊天框中</li>
                <li>• 在右侧要求补充特定场景的用例</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
          <div className="px-5 py-3 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
            <div className="flex items-center gap-2"><Bot className="w-4 h-4 text-purple-600" /><h2 className="text-base font-semibold text-gray-900">用例对话</h2>{messages.length > 0 && <span className="text-xs text-gray-400 ml-1">{messages.length} 条消息</span>}</div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-400 py-16">
                <TestTube2 className="w-12 h-12 mb-3 opacity-40" /><p className="text-sm font-medium">暂无对话记录</p><p className="text-xs mt-1">在左侧输入 PRD 后点击"生成测试用例"</p>
              </div>
          ...
          <div className="border-t border-gray-200 bg-gray-50 p-4">
            <div className="flex items-end gap-2">
              <textarea ref={chatInputRef} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleChatSend() } }}
                placeholder="对测试用例提出修改意见，例如：补充边界条件、增加安全测试..." className="flex-1 p-2.5 border border-gray-200 rounded-xl focus:border-green-500 focus:ring-2 focus:ring-green-500/10 resize-none text-sm bg-white" rows={2} />
