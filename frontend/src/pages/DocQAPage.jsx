...
import { Upload, MessageSquare, Send, Trash2, Clock, FileText, Eye, Bot, User, Sparkles, Search } from 'lucide-react'
...
export default function DocQAPage() {
  ...
  const handleUpload = async (e) => {
    ...
    if (!file) return
    setUploading(true); setMessages([])
    try {
      const form = new FormData(); form.append('file', file)
      const res = await api.post('/api/doc-qa/upload', form)
      setDocInfo(res.data); loadRecords()
  ...
  const deleteRecord = async (id) => {
    try { await api.delete(`/api/doc-qa/records/${id}`); loadRecords(); toast.success('已删除') }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <Card>
            ...
            <input ref={fileRef} type="file" accept=".pdf,.docx,.doc,.txt,.md,.csv" onChange={handleUpload} className="hidden" />
            <button onClick={() => fileRef.current?.click()} disabled={uploading}
              ...
              <div className="text-sm text-gray-500">{uploading ? '解析中...' : '点击上传文档'}</div>
                ...
                {records.map((r) => (
                  <div key={r.id} className="flex items-center justify-between p-2 rounded-lg bg-gray-50 text-xs">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-gray-400">{r.text_length}字符 · {r.created_at?.slice(0,10)}</div>
                    </div>
          ...
          <Card className="flex flex-col" style={{ minHeight: '520px' }}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-gray-900 flex items-center gap-2">
                ...
                {docInfo && <span className="text-xs text-gray-400 font-normal">| {docInfo.filename}</span>}
              </h3>
            ...
            <div ref={chatRef} className="flex-1 overflow-y-auto space-y-3 mb-4 max-h-[420px] pr-2">
              {messages.length === 0 ? (
                <Empty icon={Search} title="开始探索文档" description="上传文档后，你可以像聊天一样自由提问" />
              ) : (
                messages.map((m, i) => (
                  <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'justify-end' : ''}`}>
                    ...
                    <div className={`max-w-[78%] p-3 rounded-2xl ${
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <span className={`text-[10px] ${m.role === 'user' ? 'text-white/60' : 'text-gray-400'}`}>
