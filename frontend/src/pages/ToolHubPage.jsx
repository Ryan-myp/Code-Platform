...
const ICON_MAP = {
  ...
  GraduationCap, BookOpen, GitBranch, Layers,
  Search, UserCircle, Megaphone, TrendingUp, BarChart,
  Presentation, Table, Code, Briefcase, Mic, DollarSign, FlaskConical,
  Tag, Palette, PenTool, MessageSquare, Layout
}

const CATEGORY_COLORS = {
  '职场办公': { bg: 'from-blue-500 to-indigo-600', light: 'bg-blue-50', text: 'text-blue-600', border: 'border-blue-200' },
  '自媒体创作': { bg: 'from-pink-500 to-rose-600', light: 'bg-pink-50', text: 'text-pink-600', border: 'border-pink-200' },
  '学习研究': { bg: 'from-cyan-500 to-teal-600', light: 'bg-cyan-50', text: 'text-cyan-600', border: 'border-cyan-200' },
  '产品/营销': { bg: 'from-purple-500 to-violet-600', light: 'bg-purple-50', text: 'text-purple-600', border: 'border-purple-200' },
  '互联网行业': { bg: 'from-blue-500 to-cyan-600', light: 'bg-sky-50', text: 'text-sky-600', border: 'border-sky-200' },
  '传统行业': { bg: 'from-amber-500 to-orange-600', light: 'bg-amber-50', text: 'text-amber-600', border: 'border-amber-200' },
  '通用办公': { bg: 'from-gray-500 to-slate-600', light: 'bg-gray-50', text: 'text-gray-600', border: 'border-gray-200' },
  '运营分析': { bg: 'from-teal-500 to-emerald-600', light: 'bg-teal-50', text: 'text-teal-600', border: 'border-teal-200' },
  '金融财务': { bg: 'from-blue-600 to-indigo-700', light: 'bg-blue-50', text: 'text-blue-700', border: 'border-blue-200' },
  '人力资源': { bg: 'from-violet-500 to-purple-600', light: 'bg-violet-50', text: 'text-violet-600', border: 'border-violet-200' },
  '法律合规': { bg: 'from-red-500 to-rose-600', light: 'bg-red-50', text: 'text-red-600', border: 'border-red-200' },
  '电商运营': { bg: 'from-orange-500 to-amber-600', light: 'bg-orange-50', text: 'text-orange-600', border: 'border-orange-200' },
  '设计创意': { bg: 'from-pink-500 to-fuchsia-600', light: 'bg-pink-50', text: 'text-pink-600', border: 'border-pink-200' },
  '专业工具': { bg: 'from-orange-500 to-red-600', light: 'bg-orange-50', text: 'text-orange-600', border: 'border-orange-200' },
}
...
// 热门工具（首页推荐）
const HOT_TOOLS = ['meeting-notes', 'xiaohongshu', 'viral-title', 'competitive-analysis', 'ppt-factory', 'excel-assistant']
...
export default function ToolHubPage() {
  ...
  // 获取分类颜色
  const getCategoryStyle = (category) => CATEGORY_COLORS[category] || { bg: 'from-gray-500 to-gray-600', light: 'bg-gray-50', text: 'text-gray-600', border: 'border-gray-200' }
    ...
    <div className="flex-1 overflow-auto bg-gradient-to-b from-gray-50 to-white">
      <div className="max-w-7xl mx-auto px-6 py-8">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-white/80 text-sm mt-1">
                  {tools.length} 款专业工具 · 覆盖 {Object.keys(CATEGORY_COLORS).length} 大场景 · 让工作效率提升 10 倍
                </p>
              </div>
            </div>
        ...
        <div className="flex items-center gap-4 mb-6 border-b border-gray-200">
          <button
            onClick={() => { setActiveTab('all'); setSelectedCategory('全部') }}
            className={`pb-3 px-2 text-sm font-medium border-b-2 transition-colors ${
              activeTab === 'all' ? 'border-brand-500 text-brand-600' : 'border-transparent text-gray-500 hover:text-gray-700'
            }`}
          >
          ...
          <button
            ...
            className={`pb-3 px-2 text-sm font-medium border-b-2 transition-colors ${
              activeTab === 'favorites' ? 'border-yellow-500 text-yellow-600' : 'border-transparent text-gray-500 hover:text-gray-700'
            }`}
          >
          ...
          <button
            ...
            className={`pb-3 px-2 text-sm font-medium border-b-2 transition-colors ${
              activeTab === 'stats' ? 'border-teal-500 text-teal-600' : 'border-transparent text-gray-500 hover:text-gray-700'
            }`}
          >
        ...
        {activeTab === 'favorites' && (
          <div className="mb-8">
            {favorites.length > 0 ? (
              <div className={`grid gap-3 ${viewMode === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4' : 'grid-cols-1'}`}>
                {favorites.map(tool => {
                  const colors = getCategoryStyle(tool.category)
                  return (
                    <ToolCard
