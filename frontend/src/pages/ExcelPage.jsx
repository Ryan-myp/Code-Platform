...
const QUICK_TEMPLATES = [
  { name: '销售数据分析', icon: '📊', op: 'analyze', prompt: '请分析以下销售数据，找出销售趋势、TOP产品、区域差异，并给出提升建议' },
  { name: '用户行为分析', icon: '👥', op: 'analyze', prompt: '请分析用户行为数据，包括活跃度、留存率、转化漏斗，并给出优化建议' },
  { name: '财务报表分析', icon: '💰', op: 'analyze', prompt: '请分析以下财务数据，计算关键财务指标，评估经营状况并给出建议' },
  { name: 'VLOOKUP匹配', icon: '🔍', op: 'formula', prompt: '需要根据A列的产品编码，从另一个表中匹配对应的产品名称和价格' },
  { name: '条件汇总', icon: '📋', op: 'formula', prompt: '需要按月份和部门汇总销售额，计算平均值、最大值、最小值' },
  { name: '数据去重清洗', icon: '🧹', op: 'clean', prompt: '请检查数据中的重复行、空值、异常值，并给出清洗方案' },
]
...
export default function ExcelPage() {
  ...
  const loadHistory = async () => {
    ...
    try { const res = await api.get('/api/excel/history'); setHistory(res.data) } catch (e) { setHistoryError(e.message) }
    finally { setHistoryLoading(false) }
  }

  const execute = async () => {
    const finalInput = fileContent || dataInput || prompt
    if (!prompt.trim() && !finalInput.trim()) { toast.error('请输入内容或上传文件'); return }
    ...
    try {
      ...
      const res = await api.post('/api/excel/operate', {
        operation, title: prompt.slice(0, 50) || uploadedFile?.name || '未命名', data
      })
      setResult(res.data.result); loadHistory(); toast.success('操作完成')
  ...
  const handleRemoveFile = () => {
    setUploadedFile(null); setFileContent(''); setDataInput(''); setPreviewData(null)
    if (fileInputRef.current) fileInputRef.current.value = ''
  }

  const applyQuickTemplate = (tpl) => {
    setOperation(tpl.op); setPrompt(tpl.prompt)
    toast.success(`已应用模板：${tpl.name}`)
  }

  const reuseHistory = (item) => {
    setOperation(item.operation); setResult(item.result)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: '总操作数', value: history.length, icon: Table2, color: 'from-green-500 to-emerald-600' },
          { label: '本周使用', value: history.filter(h => (new Date() - new Date(h.created_at)) < 7 * 86400000).length, icon: Sparkles, color: 'from-purple-500 to-indigo-600' },
          { label: '操作类型', value: `${OPERATIONS.length}种`, icon: Calculator, color: 'from-blue-500 to-cyan-600' },
          { label: '快速模板', value: QUICK_TEMPLATES.length, icon: FileText, color: 'from-amber-500 to-orange-600' },
        ].map((s, i) => (
      ...
      <Card>
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
          {QUICK_TEMPLATES.map((tpl, i) => (
            <button key={i} onClick={() => applyQuickTemplate(tpl)}
          ...
          <Card>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1.5">{OPERATIONS.find(o => o.value === operation)?.desc}</p>
              </div>
              ...
              {operation === 'formula' ? (
                <div>
                  ...
                  <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)}
                    ...
                    rows={5} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none" />
                </div>
              ) : (
                <div>
                  ...
                  <div className="mb-3">
                    {uploadedFile ? (
                      <div className="flex items-center gap-2 px-3 py-2 bg-green-50 border border-green-200 rounded-lg">
                        ...
                        <button onClick={handleRemoveFile} className="text-gray-400 hover:text-red-500"><X className="w-4 h-4" /></button>
                      </div>
                    ) : (
                  ...
                  <textarea value={dataInput || prompt} onChange={(e) => setDataInput(e.target.value)}
                    placeholder={operation === 'analyze' ? '粘贴 Excel 数据（可用 Tab 或逗号分隔）或上传文件...' : '输入需要创建的数据...'}
                    rows={5} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none font-mono" />
                </div>
              )}

              {/* 补充说明 */}
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">补充说明（可选）</label>
              ...
              <Button variant="success" icon={Play} loading={loading} onClick={execute} className="w-full">执行</Button>
            </div>
          </Card>
        </div>
                  ...
                  <thead>
                    <tr className="bg-gray-50">
                      {previewData.headers.map((h, i) => (
                        <th key={i} className="px-2 py-1.5 text-left font-medium text-gray-600 border-b">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {previewData.rows.map((row, ri) => (
                      <tr key={ri} className="hover:bg-gray-50">
                        {row.map((cell, ci) => (
                          <td key={ci} className="px-2 py-1 text-gray-700 border-b border-gray-100">{cell}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
          ...
          <div className="space-y-2">
            {history.slice(0, 10).map(item => (
              <div key={item.id} className="flex items-center gap-3 p-3 rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer transition-colors"
                ...
                <span className="text-sm text-gray-700 truncate flex-1">{item.title || '未命名'}</span>
