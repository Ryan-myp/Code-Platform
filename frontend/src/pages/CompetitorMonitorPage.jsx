...
import {
  Plus, Trash2, Loader2, Radar, TrendingUp, Target, Clock, Search,
  Activity, ThumbsUp, MessageSquare, Share2, AlertTriangle, CheckCircle2,
} from 'lucide-react'
...
export default function CompetitorMonitorPage() {
  ...
  const [form, setForm] = useState({ name: '', platform: '抖音', account_id: '', description: '', profile_url: '' })
  ...
  const handleAdd = async () => {
    if (!form.name.trim()) { toast.error('请填写竞品名称'); return }
    setSaving(true)
    try {
  ...
  const handleAnalyze = async () => {
    if (selectedIds.length === 0) { toast.error('请至少选择一个竞品'); return }
    setAnalyzing(true)
    try {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <Button variant="primary" icon={Plus} onClick={() => setAddOpen(true)}>添加竞品</Button>
        }
      />

      {loading ? <PageLoading /> : error ? <ErrorState message={error} onRetry={fetchAll} /> : (
        <>
          {/* 竞品列表 */}
          <Card>
            ...
            {competitors.length === 0 ? (
              <Empty
                ...
                action={<Button icon={Plus} onClick={() => setAddOpen(true)}>添加第一个竞品</Button>}
              />
            ) : (
              <div className="space-y-2">
                {competitors.map((c) => (
                  <div key={c.id} className="flex items-center gap-3 p-3 rounded-xl border border-ink-100 hover:border-brand-200 hover:bg-brand-50/30 transition-all">
                    <input
                      type="checkbox"
                    ...
                    />
                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-xs font-semibold flex-shrink-0 ${PLATFORM_COLORS[c.platform] || 'bg-gray-500 text-white'}`}>
                      {c.platform?.[0] || '竞'}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-ink-900 truncate">{c.name}</p>
                      <p className="text-xs text-ink-400 truncate">
                        {c.platform} · {c.account_id || c.profile_url || c.description || '未填写账号'}
                      </p>
                    </div>
                    <span className="text-[11px] text-ink-300 hidden sm:block">更新于 {formatDateTime(c.updated_at)}</span>
                    <button
          ...
          <Card>
            ...
            {reports.length === 0 ? (
              <Empty icon={Radar} title="暂无分析报告" description="选择竞品并点击分析，生成策略洞察与对比雷达图" />
            ) : (
                ...
                {reports.map((r) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-ink-600 line-clamp-2">
                      竞品数 {(() => { try { return JSON.parse(r.competitor_ids || '[]').length } catch { return 0 } })()} · 点击查看详情
                    </p>
                  </button>
                ))}
              </div>
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="添加竞品">
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">账号 ID / 主页链接</label>
            <input
        ...
        <div className="flex justify-end gap-2 mt-6">
          ...
          <Button variant="primary" icon={Plus} loading={saving} onClick={handleAdd}>添加</Button>
        </div>
      </Modal>
        ...
        {report ? (
          <div className="space-y-5 max-h-[70vh] overflow-y-auto pr-1">
            <div className="flex items-center gap-2 text-xs text-ink-400">
              ...
              <span className="ml-auto flex items-center gap-1 text-emerald-600"><CheckCircle2 className="w-3.5 h-3.5" /> AI 完成</span>
            </div>
                  ...
                  {report.analysis.content_categories.map((c, i) => (
                    <div key={i} className="flex items-center gap-3 text-sm">
                      ...
                      <div className="flex-1 h-2.5 bg-gray-100 rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-brand-500 to-indigo-500 rounded-full" style={{ width: `${Math.min(c.percentage || 0, 100)}%` }} />
                      </div>
                ...
                <ul className="space-y-1.5">
                  {report.analysis.hot_patterns.map((p, i) => (
                    <li key={i} className="text-sm text-ink-600 flex gap-2">
                      <span className="text-rose-400 flex-shrink-0">◆</span>{p}
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                <div className="p-4 rounded-xl border border-ink-100">
                  ...
                  <p className="text-xs text-ink-500 leading-relaxed">
                    频率：{report.analysis.publishing_habits.frequency || '—'}<br />
                    主平台：{report.analysis.publishing_habits.platform_focus || '—'}<br />
                    最佳时段：{(report.analysis.publishing_habits.best_times || []).join('、') || '—'}<br />
                ...
                <div className="p-4 rounded-xl border border-ink-100">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div><ThumbsUp className="w-4 h-4 text-brand-500 mx-auto mb-1" /><p className="text-sm font-semibold">{report.analysis.engagement_analysis?.avg_likes ?? '—'}</p><p className="text-[10px] text-ink-400">均赞</p></div>
                    <div><MessageSquare className="w-4 h-4 text-emerald-500 mx-auto mb-1" /><p className="text-sm font-semibold">{report.analysis.engagement_analysis?.avg_comments ?? '—'}</p><p className="text-[10px] text-ink-400">均评</p></div>
                    <div><Share2 className="w-4 h-4 text-amber-500 mx-auto mb-1" /><p className="text-sm font-semibold">{report.analysis.engagement_analysis?.avg_shares ?? '—'}</p><p className="text-[10px] text-ink-400">均转</p></div>
                  </div>
                  <p className="text-xs text-ink-500 mt-2">互动率：{report.analysis.engagement_analysis?.engagement_rate || '—'} · 趋势：{report.analysis.engagement_analysis?.trend || '—'}</p>
                </div>
              </div>
            )}
                ...
                <div className="p-4 rounded-xl border border-emerald-100 bg-emerald-50/50">
                  ...
                  <ul className="space-y-1">
                    {report.analysis.competitive_advantages.map((a, i) => (
                      <li key={i} className="text-xs text-emerald-700 flex gap-1.5"><span>•</span>{a}</li>
                    ))}
                  </ul>
                </div>
              )}
                ...
                <div className="p-4 rounded-xl border border-amber-100 bg-amber-50/50">
                  ...
                  <ul className="space-y-1">
                    {report.analysis.competitive_weaknesses.map((a, i) => (
                      <li key={i} className="text-xs text-amber-700 flex gap-1.5"><span>•</span>{a}</li>
                    ))}
                  </ul>
                </div>
              )}
                ...
                <ol className="space-y-1.5">
                  {report.analysis.recommendations.map((r, i) => (
                    <li key={i} className="text-xs text-brand-700 flex gap-2">
                      <span className="w-4 h-4 rounded-full bg-brand-500 text-white text-[10px] flex items-center justify-center flex-shrink-0">{i + 1}</span>
                      {r}
                    </li>
                  ))}
                </ol>
            ...
            {radarIndicators.length > 0 && (
              <div className="p-4 rounded-xl border border-ink-100">
                <h4 className="text-sm font-semibold text-ink-900 mb-1 flex items-center gap-1.5">
                  <Radar className="w-4 h-4 text-indigo-500" /> {report.radar?.title || '竞品对比雷达'}
                </h4>
                {report.radar?.insight && <p className="text-xs text-ink-500 mb-3">{report.radar.insight}</p>}
                <div className="space-y-2">
                          ...
                          return (
                            <div key={si} className="flex-1 min-w-[80px]">
                              <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                                <div className={`h-full rounded-full ${si === 0 ? 'bg-gradient-to-r from-brand-500 to-indigo-500' : 'bg-gradient-to-r from-rose-400 to-orange-400'}`} style={{ width: `${pct}%` }} />
                              </div>
                              <p className="text-[10px] text-ink-400 mt-0.5 truncate">{s.name}: {val}</p>
                            </div>
                          )
                        })}
                      </div>
