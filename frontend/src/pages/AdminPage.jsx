...
export default function AdminPage() {
  ...
  const loadOrders = async () => {
    try {
      const res = await api.get('/api/admin/orders', {
        params: { status: orderFilter === 'all' ? '' : orderFilter },
      })
      setOrders(res.data)
    } catch (err) {
  ...
  const handleReview = async () => {
    ...
    setReviewing(true)
    try {
      await api.post(`/api/admin/orders/${reviewTarget.order.id}/review`, {
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
            {statCards.map((card) => (
              <div
                key={card.label}
                className="bg-white rounded-2xl border border-ink-200/60 shadow-soft p-4"
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-2xl border border-ink-200/60 shadow-soft p-5 lg:col-span-2">
              <h3 className="font-semibold text-ink-900 flex items-center gap-2 mb-4">
                <BarChart3 className="w-4 h-4 text-brand-500" />近 7 天调用趋势
              </h3>
                  ...
                  {users.map((u) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3 pr-4">
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs ${ROLE_META[u.role]?.cls || 'bg-ink-100 text-ink-600'}`}
                          >
                        ...
                        <td className="py-3 pr-4">
                          <span className={`px-2 py-0.5 rounded-full text-xs ${meta.badge}`}>
                            {u.membership === 'vip' && (
                              <Crown className="w-3 h-3 inline mr-0.5 text-amber-500" />
                            )}
                            {meta.label}
                          </span>
                          {u.active === 0 && (
                            <span className="ml-1.5 px-2 py-0.5 rounded-full text-xs bg-red-50 text-red-500">
                              已禁用
                            </span>
                          )}
                        </td>
          ...
          <div className="bg-white rounded-2xl border border-ink-200/60 shadow-soft p-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-6 gap-3 mb-4">
              ...
              <input
                ...
                onChange={(e) =>
                  setCouponForm({ ...couponForm, expires_days: Number(e.target.value) })
                }
                  ...
                  <tbody>
                    {coupons.map((c) => (
                      <tr key={c.id} className="border-b border-ink-50 hover:bg-ink-50/40">
                        <td className="py-2.5 pr-3 font-mono font-semibold text-brand-600">
                          {c.code}
                        </td>
                        ...
                        <td className="py-2.5 pr-3">
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs ${c.active ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-400'}`}
                          >
                            {c.active ? '启用' : '停用'}
                          </span>
                        </td>
                    ...
                    <tbody>
                      {shareStats.shares.map((s) => (
                        <tr key={s.id} className="border-b border-ink-50 hover:bg-ink-50/40">
                          <td className="py-2.5 pr-3 text-ink-700 max-w-[200px] truncate">
                            {s.title}
                          </td>
                            ...
                            <div className="flex flex-wrap gap-1">
                              {s.sources.length === 0 ? (
                                ...
                                s.sources.slice(0, 3).map((src) => (
                                  <span
                                    key={src.source}
                                    className="px-1.5 py-0.5 rounded bg-purple-50 text-purple-600 text-[11px]"
                                  >
              ...
              <>
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-5">
                  {[
                    ...
                    {
                      label: '开通转化率',
                      value: `${orderStats.conversion_rate}%`,
                      color: 'text-emerald-600',
                    },
                    { label: '客单价', value: `¥${orderStats.avg_order}`, color: 'text-brand-600' },
                    {
                      label: '优惠券抵扣',
                      value: `¥${orderStats.discount_total}`,
                          ...
                          >
                            ...
                            <div
                              ...
                              style={{
                                height:
                                  t.revenue > 0
                                    ? `${Math.max(4, (t.revenue / maxRev) * 100)}%`
                                    : '2px',
                              }}
                            />
                          </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {Object.entries(STATUS_LABELS).map(([k, label]) => (
                          <span
                            key={k}
                            className="px-2.5 py-1 rounded-lg bg-ink-50 text-xs text-ink-600"
                          >
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-page-in">
                ...
                <p className="text-xs text-ink-400 mb-5">
                  @{editingUser.username} · 调整角色、会员等级与每日额度
                </p>
...
function ContentManager() {
  ...
  const changeVisibility = async (item, visibleTo) => {
    ...
    try {
      ...
      toast.success(
        `「${item.name}」已设为「${VISIBLE_TO_OPTIONS.find((o) => o.value === visibleTo)?.label || visibleTo}」`
      )
      setItems((prev) =>
        prev.map((i) => (i.resource_id === item.resource_id ? { ...i, visible_to: visibleTo } : i))
      )
    } catch (err) {
      toast.error(err.message || '更新失败')
    } finally {
  ...
  const filtered = items.filter(
    (i) =>
      ...
      i.resource_id.toLowerCase().includes(search.toLowerCase())
  )
...
function DigitalHumanStats() {
  ...
  const cards = [
    {
      ...
      sub: `今日 +${totals.today ?? 0}`,
      color: 'from-brand-500 to-indigo-500',
    },
    {
      label: '生成成功率',
      value: `${Math.round((usage.success_rate || 0) * 100)}%`,
      sub: `${usage.success || 0} / ${usage.total || 0} 次成功`,
      color: 'from-green-500 to-emerald-500',
    },
    {
      label: '平均耗时',
      value: `${usage.avg_seconds ?? 0}s`,
      sub: '单条生成（含渲染）',
      color: 'from-amber-500 to-orange-500',
    },
    {
      label: '存储占用',
      value: `${storage.total_mb ?? 0}MB`,
      sub: `音频 ${storage.audio_count || 0} / 视频 ${storage.video_count || 0}`,
      color: 'from-purple-500 to-fuchsia-500',
    },
    {
      label: '批量任务',
      value: batches.total ?? 0,
      sub: `完成 ${batches.done ?? 0} / 中断 ${batches.interrupted ?? 0}`,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-ink-500">
          数字人内容工厂 · 生产运营实时指标（成功率/耗时来自 usage_logs）
        </p>
        <button
          onClick={load}
          className="flex items-center gap-2 px-3 py-2 text-sm rounded-xl border border-ink-200 hover:bg-ink-50 transition-colors"
          ...
          <div className="bg-white rounded-2xl border border-ink-200/60 shadow-soft p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(statusDist).map(([k, v]) => (
                <span
                  key={k}
                  className={`px-3 py-1.5 rounded-full text-sm ${statusMeta[k]?.cls || 'bg-gray-100 text-gray-600'}`}
                >
          ...
          <div className="bg-white rounded-2xl border border-ink-200/60 shadow-soft p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(resDist).map(([k, v]) => (
                <span
                  key={k}
                  className="px-3 py-1.5 rounded-full text-sm bg-indigo-50 text-indigo-600"
                >
