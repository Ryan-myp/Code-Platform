...
export default function GrowthPage() {
  ...
  const getSuggestions = async (hotspot) => {
    ...
    try {
      const res = await api.post('/api/strategy/topic-suggest', {
        hotspot: hotspot.title, platform, source: hotspot.source,
      })
      setSuggestions(res.data)
    } catch (e) { toast.error(`选题建议生成失败：${e.message}`) }
  ...
  const createSeries = async () => {
    if (!seriesForm.name.trim()) { toast.error('请输入系列名称'); return }
    try {
  ...
  const addComment = async () => {
    if (!commentRecordId.trim()) { toast.error('请先输入发布记录 ID'); return }
    if (!commentForm.content.trim()) { toast.error('请输入评论内容'); return }
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card>
              ...
              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">发布记录 ID</label>
                  <div className="flex gap-2">
                    <input type="text" value={commentRecordId} onChange={(e) => setCommentRecordId(e.target.value)}
                      placeholder="如：pub_abc123def4"
                    ...
                    <Button variant="secondary" size="sm" icon={RefreshCw} onClick={loadComments}>刷新</Button>
                  </div>
                </div>
                {/* 手动添加评论 */}
                <div className="p-3 rounded-lg bg-gray-50 border border-gray-100">
                  ...
                  <textarea value={commentForm.content} onChange={(e) => setCommentForm({ ...commentForm, content: e.target.value })}
                    placeholder="评论内容…" rows={2}
                    className="w-full px-3 py-1.5 border border-gray-200 rounded-lg text-xs mb-2 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none" />
                  <Button variant="primary" size="sm" icon={Plus} loading={commentSaving} onClick={addComment} className="w-full">添加评论</Button>
                </div>
              </div>
            </Card>
            ...
            <Card className="lg:col-span-2">
              ...
              {!commentRecordId ? (
                <Empty icon={MessageCircle} title="输入发布记录 ID" description="在左侧输入发布记录 ID 查看对应评论" />
              ) : comments.length === 0 ? (
                <Empty icon={MessageCircle} title="暂无评论" description="可手动录入评论或等待平台 API 自动拉取" />
              ) : (
                <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
                  {comments.map((c) => (
                    <div key={c.id} className={`p-3 rounded-xl border transition-all ${
                      c.replied ? 'border-emerald-200 bg-emerald-50/30' : 'border-gray-100 hover:border-purple-200'
                    }`}>
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-medium text-gray-700">{c.author || '匿名用户'}</span>
                          {c.likes > 0 && <span className="text-[10px] text-gray-400 flex items-center gap-0.5"><ThumbsUp className="w-2.5 h-2.5" />{c.likes}</span>}
                      ...
                      {c.reply_content ? (
                        <div className="p-2 rounded-lg bg-emerald-50 border border-emerald-100 text-xs text-emerald-700">
                          <span className="font-medium">AI 回复：</span>{c.reply_content}
                        </div>
                      ) : (
          ...
          <div className="lg:col-span-2">
            <Card>
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  <select value={hotSource} onChange={(e) => setHotSource(e.target.value)}
                    className="px-2.5 py-1.5 rounded-lg border border-gray-200 text-xs text-gray-600 bg-white focus:outline-none">
              ...
              {hotLoading ? (
                ...
                <div className="space-y-2 max-h-[600px] overflow-y-auto pr-1">
                  {hotspots.map((h) => (
                    <div key={h.global_rank} className={`p-3 rounded-xl border transition-all cursor-pointer ${
                      suggestId === h.title ? 'border-orange-300 bg-orange-50/30' : 'border-gray-100 hover:border-orange-200 hover:bg-orange-50/20'
                    }`} onClick={() => getSuggestions(h)}>
                      <div className="flex items-start gap-3">
                        <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5 ${
                          h.global_rank <= 3 ? 'bg-orange-500 text-white' : h.global_rank <= 10 ? 'bg-orange-100 text-orange-600' : 'bg-gray-100 text-gray-500'
                        }`}>{h.global_rank}</span>
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-medium text-gray-800">{h.title}</div>
                          <div className="flex items-center gap-2 mt-1">
                            <Badge color={h.source === 'weibo' ? 'red' : h.source === 'zhihu' ? 'blue' : 'green'}>{h.source_label}</Badge>
                            <span className="text-[10px] text-gray-400">热度 {(h.heat / 10000).toFixed(0)}万</span>
                          </div>
                        </div>
            ...
            {suggestLoading ? (
              ...
              <div className="space-y-3">
                ...
                {suggestions.suggestions?.map((s, i) => (
                  <div key={i} className="p-3 rounded-lg border border-gray-100 hover:border-purple-200 transition-all">
                    <div className="text-sm font-semibold text-gray-800 mb-1">{s.title_direction}</div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Button variant="secondary" size="sm" icon={ArrowRight}
                        onClick={() => importToVariants(s)}>导入变体</Button>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <Empty icon={Flame} title="点击左侧热点" description="AI 将为你生成选题角度和标题方向" />
            )}
          </Card>
        </div>
      )}
      ...
      {tab === 'series' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card>
            ...
            <div className="space-y-3">
              <input type="text" value={seriesForm.name} onChange={(e) => setSeriesForm({ ...seriesForm, name: e.target.value })}
              ...
              <textarea value={seriesForm.description} onChange={(e) => setSeriesForm({ ...seriesForm, description: e.target.value })}
                placeholder="系列简介（可选）" rows={2}
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none" />
              <select value={seriesForm.platform} onChange={(e) => setSeriesForm({ ...seriesForm, platform: e.target.value })}
              ...
              <Button variant="primary" icon={Plus} onClick={createSeries} className="w-full">创建系列</Button>
            </div>
          </Card>
          ...
          <div className="lg:col-span-2 space-y-4">
            <Card>
              ...
              {series.length === 0 ? (
                <Empty icon={Layers} title="暂无内容系列" description="创建系列来组织你的发布内容，便于统一管理和效果对比" />
              ) : (
                <div className="space-y-2">
                  {series.map((s) => (
                    <div key={s.id} className={`p-3 rounded-xl border transition-all cursor-pointer ${
                      seriesDetail === s.id ? 'border-indigo-300 bg-indigo-50/20' : 'border-gray-100 hover:border-indigo-200'
                    }`} onClick={() => loadSeriesStats(s.id)}>
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="text-sm font-medium text-gray-800">{s.name}</div>
                          <div className="text-xs text-gray-400">{s.item_count || 0} 篇 · {s.platform ? PLATFORMS.find((p) => p.value === s.platform)?.label || s.platform : '全平台'}</div>
                        </div>
                ...
                <div className="grid grid-cols-3 gap-3 mb-4">
                  {[
                    ...
                    <div key={i} className="text-center p-2 rounded-lg bg-gray-50">
                      <div className="text-lg font-bold text-gray-800">{m.value.toLocaleString()}</div>
