...
import { Code2, Send, Loader2, CheckCircle2, AlertCircle, Database, GitBranch, Layers, Settings, Copy, Sparkles, Bot, User, Edit2, Save, X } from 'lucide-react'
...
export default function TDPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await axios.post('/api/prd/technical-design', { prd_text: stripImages(prdText), repo_path: repoPath })
      const result = res.data.result || res.data.document || '生成失败'
  ...
  const handleChatSend = async () => {
    ...
    try {
      ...
      const res = await axios.post('/api/prd/technical-design', { prd_text: fullContext, repo_path: repoPath })
      setMessages(prev => [...prev, { role: 'assistant', content: res.data.result || res.data.document || '处理失败', timestamp: new Date().toISOString() }])
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center space-y-4 py-6">
        <h1 className="text-3xl font-bold text-gray-900 flex items-center justify-center gap-2"><Code2 className="w-8 h-8 text-indigo-600" /> 技术方案</h1>
        <p className="text-lg text-gray-600">基于 PRD 生成完整的技术设计方案，支持对话式迭代优化</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-[1600px] mx-auto" style={{ height: 'calc(100vh - 280px)' }}>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
          <div className="px-5 py-3 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
            <h2 className="text-base font-semibold text-gray-900 flex items-center gap-2"><Settings className="w-4 h-4 text-indigo-600" />配置</h2>
            <span className="text-xs text-gray-400">左侧：输入 PRD，点击生成</span>
          </div>
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            <RichTextEditor value={prdText} onChange={setPrdText} placeholder={"请输入 PRD 内容..."} minHeight={180} />
            <div>
            ...
            <button onClick={handleGenerate} disabled={loading || !prdText.trim()}
              className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-2.5 px-4 rounded-lg hover:from-indigo-700 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium transition-all flex items-center justify-center gap-2">
              {loading ? <><Loader2 className="w-4 h-4 animate-spin" />生成中...</> : <><Sparkles className="w-4 h-4" />生成技术方案</>}
            </button>
            <div className="p-3 bg-indigo-50 rounded-lg border border-indigo-100">
              <p className="text-xs font-medium text-indigo-700 mb-1">💡 使用提示</p>
              <ul className="text-xs text-indigo-600 space-y-0.5">
                <li>• 在左侧输入 PRD，点击"生成技术方案"</li>
                <li>• 方案出现在右侧聊天框中</li>
                <li>• 在右侧对方案提出修改意见</li>
              </ul>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
          <div className="px-5 py-3 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
            <div className="flex items-center gap-2"><Bot className="w-4 h-4 text-purple-600" /><h2 className="text-base font-semibold text-gray-900">方案对话</h2>{messages.length > 0 && <span className="text-xs text-gray-400 ml-1">{messages.length} 条消息</span>}</div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-400 py-16">
                <Layers className="w-12 h-12 mb-3 opacity-40" /><p className="text-sm font-medium">暂无对话记录</p><p className="text-xs mt-1">在左侧输入 PRD 后点击"生成技术方案"</p>
              </div>
          ...
          <div className="border-t border-gray-200 bg-gray-50 p-4">
            <div className="flex items-end gap-2">
              <textarea ref={chatInputRef} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleChatSend() } }}
                placeholder="对技术方案提出修改意见..." className="flex-1 p-2.5 border border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 resize-none text-sm bg-white" rows={2} />
