...
export default function useAsyncTask() {
  ...
  const startPolling = useCallback((taskId, { onUpdate, onSuccess, onError, interval = 3000 }) => {
    stopPolling()
    ...
    wsUnsubRef.current = connectWs(`task:${taskId}`, {
      ...
      onMessage: (msg) => {
        ...
        if (event === 'task_update') {
          ...
          api.get(`/api/tasks/${taskId}`).then(
            (res) => finish(onError, {
              message: res.data?.error || (event === 'task_canceled' ? '任务已取消' : '任务执行失败'),
              task: res.data,
            }),
            () => finish(onError, { message: event === 'task_canceled' ? '任务已取消' : '任务执行失败' })
          )
        }
      },
    })
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (t.status === 'success') {
          ...
          finish(onError, { message: t.error || (t.status === 'canceled' ? '任务已取消' : '任务执行失败'), task: t })
        }
      } catch {
        // 网络抖动：保留轮询，下次继续
      }
