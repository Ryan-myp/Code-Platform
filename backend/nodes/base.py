...
class NodeResult:
    ...
    def __init__(self, status: str, output: Dict[str, Any] = None, 
                 messages: Optional[List[str]] = None, error: Optional[str] = None):
    ...
    @classmethod
    def success(cls, output: Dict[str, Any], messages: Optional[List[str]] = None) -> 'NodeResult':
        return cls(status="success", output=output, messages=messages or [])
    
    @classmethod
    def failed(cls, error: str, output: Optional[Dict[str, Any]] = None, 
               messages: Optional[List[str]] = None) -> 'NodeResult':
...
class PrintNode(BusinessNode):
    ...
    def execute(self, context: Dict[str, Any]) -> NodeResult:
        ...
        return NodeResult.success(
            output={"echo": current_input},
            messages=[f"已打印 {self.node_id} 的输入"]
        )
