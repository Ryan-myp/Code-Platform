...
def _load_one(tid: str) -> dict:
    ...
    if not path.exists():
        raise HTTPException(404, "思维导图模板不存在")
...
def _get_usage(tid: str) -> int:
    try:
        ...
        conn = get_db()
        row = conn.execute(
            "SELECT usage_count FROM mindmap_template_usage WHERE template_id=?", (tid,)
        ).fetchone()
        conn.close()
...
def record_usage(tid: str) -> None:
    """生成思维导图时记录模板热度（worker 调用，失败静默）。"""
    try:
        from common.db import get_db

        conn = get_db()
        conn.execute(
            "CREATE TABLE IF NOT EXISTS mindmap_template_usage "
            "(template_id TEXT PRIMARY KEY, usage_count INTEGER DEFAULT 0)"
        )
        conn.execute(
            "INSERT INTO mindmap_template_usage(template_id, usage_count) VALUES(?,1) "
