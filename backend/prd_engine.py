...
要求：
1. 输出完整可运行的工程代码，包含必要的 import 和入口（监听 0.0.0.0:8000，根路径返回 200）
2. 代码必须可运行，错误处理完善
3. 每个文件开头注释说明文件用途
4. 多文件输出规范：每个文件单独一个 Markdown 代码块，块头格式为 ```语言 文件路径（如 ```python main.py、```javascript server.js、```go main.go），
   支持目录层级（如 ```python src/utils.py）；单文件也按此格式输出（```python main.py）
5. 若语言为 python，同时输出依赖清单（在末尾 ```text requirements.txt 代码块中列出 pip 依赖，每行一个）
直接输出代码，不要解释。"""
