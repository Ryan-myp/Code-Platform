...
def _resolve_main_slot_key(template: dict) -> str:
    """解析模板主槽位key。"""
    for layer in template.get("layers", []):
        if layer.get("type") == "image" and (layer.get("slot") or not layer.get("url")):
...
def _resolve_layer_image(layer: dict, slot_map: dict, main_slot_key: str, batch_url: str) -> str:
    """解析单个图层图片来源。"""
    key = layer.get("key", "")
    if key and key in slot_map:
        return slot_map[key]
...
def _create_background(width: int, height: int, overrides: dict, template: dict):
    """创建背景图层。"""
...
def _render_rectangle_layer(draw: ImageDraw, layer: dict, width: int, height: int):
    """渲染矩形图层（圆角、渐变、描边）。"""
    x = int(layer.get("x", 0))
    y = int(layer.get("y", 0))
    ...
    if rotation:
        ...
        nw, nh = overlay.size
        return overlay, (int(x + w / 2 - nw / 2), int(y + h / 2 - nh / 2))
