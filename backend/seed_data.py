...
def _insert_workflows(conn):
    ...
    wf3_nodes = [
        {"id": "n1", "type": "agent", "label": "开始", "x": 80, "y": 160, "config": {}},
        {
            ...
            "x": 260,
            "y": 160,
            "config": {"url": "/api/config", "method": "GET"},
        },
        {
            "id": "n3",
            "type": "code",
            ...
            "x": 440,
            "y": 160,
            "config": {"code": "print('deploy')", "language": "python"},
        },
        {
            "id": "n4",
            "type": "http",
            ...
            "x": 620,
            "y": 160,
            "config": {"url": "/api/deploy", "method": "POST"},
        },
        {"id": "n5", "type": "output", "label": "结束", "x": 800, "y": 160, "config": {}},
    ]
    wf3_edges = [
    ...
    workflows = [
        ("wf-1", "标准研发流程", "需求→设计→编码→测试→上线", json.dumps(wf1_nodes), json.dumps(wf1_edges), _NOW, 1),
        ("wf-2", "架构驱动研发", "架构设计→方案评审→代码实现", json.dumps(wf2_nodes), json.dumps(wf2_edges), _NOW, 1),
        ("wf-3", "自动化部署流程", "配置读取→脚本生成→沙箱部署", json.dumps(wf3_nodes), json.dumps(wf3_edges), _NOW, 1),
    ]
    for w in workflows:
...
def _insert_notifications(conn):
    notifications = [
        (
            ...
            "info",
            "欢迎使用小团智能平台 v9.0",
            "平台已升级到 v9.0，新增了工作台、任务中心和通知中心等功能",
            "",
            "",
            "all",
            _NOW,
